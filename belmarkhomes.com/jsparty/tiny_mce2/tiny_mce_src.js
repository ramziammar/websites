...
TinyMCE_Engine.prototype = {
	init : function(settings) {
		...
		if (!tinyMCE.baseURL) {
			...
			// This has been hard-coded because of new javascript-combining code
			if(!tinyMCE.baseURL) {
				tinyMCE.baseURL = baseHREF + 'jsparty/tiny_mce2';
				tinyMCE.srcMode = '_src';
				tinyMCE.gzipMode = false;
				tinyMCE.selfIncludedScripts = true;
			}
		}
	...
	loadScript : function(url) {
		if(tinyMCE.selfIncludedScripts) return;
		if(tinyMCE.baseURL == null) return;
	...
	execInstanceCommand : function(editor_id, command, user_interface, value, focus) {
		...
		if(typeof editor_id == 'object') inst = editor_id; 
		else inst = tinyMCE.getInstanceById(editor_id);
	...
	_createIFrame : function(replace_element, doc, win) {
		...
		tinyMCE.minHeight = parseInt(ah);
		tinyMCE.minHeight = parseInt(aw);
		...
		// fix for Gecko problem in strict mode
		if (tinyMCE.settings.strict_loading_mode)
...
/*				if (tinyMCE.selectedInstance)
	...
	onLoad : function() {
		...
		// correctly initialized in IE so this workaround solves the problem
...
/*			if (elm == inst.lastTriggerEl)
		...
		// This should be done with an observer.
		// This was raising an error during initialisation.
		try {    
...
TinyMCE_Control.prototype = {
	...
	select : function() {
		...
		// SilverStripe function name that performs a similar set of functionality
		this.focus();
	},
...
//		return this.contentDocument ? this.contentDocument : this.contentWindow.document; // Removed due to IE 5.5 ?
	...
	resizeToContent : function() {
		...
		// SilverStripe modification - minimum height of 220
		var h = (tinyMCE.isRealIE) ? b.scrollHeight : de.offsetHeight;
		
		h = (h < 220) ? '220px' : h + 'px';
		this.iframeElement.style.height = h;
	},
	...
	isDirty : function() {
		...
		return tinyMCE.trim(this.startContent) != tinyMCE.trim(this.getBody().innerHTML).replace(/ mce_serialized="[^"]+"/g,'') && !this.isNotDirty;
	},
	...
	_onAdd : function(replace_element, form_element_name, target_document) {
		...
		// SilverStripe fix - make typography work in IE
		if (true || dynamicIFrame) {
	...
	triggerSave : function(skip_cleanup, skip_callback) {
		...
		if (this.formElement) {
			...
			this.formElement.inst = this; 
			this.formElement.isChanged = function () {  
				return this.inst.isDirty(); 
			}
			this.formElement.resetChanged = function () {
				this.inst.isNotDirty = true;
			}
		}
	...
	blur : function(immediate) { 
		var te = this.getToolbarElement(); 
		if(te && te.removeFocus) te.removeFocus(); 
		this.focused = false; 
	},
	
	blurUnlessFocused : function() { 
...
tinyMCE.add(TinyMCE_Engine, {
	...
	_cleanupHTML : function(inst, doc, config, elm, visual, on_save, on_submit, inn) {
		...
		// SilverStripe fix: turn style="width: 100px; height: 150px" into width="100" height="150"
		// This is needed for IE resizing
		var i,imgs = doc.getElementsByTagName('img');
		for(i=0;i<imgs.length;i++) {
			if(imgs[i].style.width) imgs[i].width = parseInt(imgs[i].style.width);
			if(imgs[i].style.height) imgs[i].height = parseInt(imgs[i].style.height);
		}
		...
		if (a) {
			for (n in a) {
				if (typeof(a[n]) != 'function' && a[n] != null)
					// SilverStripe modification : make sure the css class is set properly
					// Can't just change the reference from 'className' to 'class' elsewhere, because
					// it makes IE cry. Alligator tears.
					if(n == 'className') {
						a['class'] = a['className'];
						n = 'class';
					}
...
TinyMCE_Selection.prototype = {
	...
	isCollapsed : function() {
		...
		// SilverStripe modification: fixed bug when loading pages via ajax in safari
		if (!r || r.item)
			return false;
