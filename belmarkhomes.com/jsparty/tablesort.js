/*
		...
		Modified and optimized by Quin Hoxie
...
function sortDatetime(a,b) {
	...
	var ai = parseFloat(new Date(aa).getTime());
	if(isNaN(ai)) {
		x = 0;
	} else {
		x = ai;
	...
	var bi = parseFloat(new Date(bb).getTime());
	if(isNaN(bi)) {
		y = 0;
	} else {
		y = bi;
...
var tablePaginater = {
        ...
        createPageinationList: function(tableId) {
                ...
                li.onclick = tablePaginater.show;
                ...
                for(var i = 0; i < pages; i++) {
                        ...
                        lic.onclick = lic.getElementsByTagName("p")[0].onclick = tablePaginater.show;
        ...
        show: function(e) {
                var li = (this.tagName && this.tagName == "P") ? this.parentNode : this;
