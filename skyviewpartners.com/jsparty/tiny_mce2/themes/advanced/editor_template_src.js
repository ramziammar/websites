...
var TinyMCE_AdvancedTheme = {
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		// SS: Use a single element for the toolbar across all items
		// Part of the manually_placed toolbar placement
		...
		// Reset old states
		// SS: toolbar_id instead of editor_id
		tinyMCE.switchClass(toolbar_id + '_justifyleft', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_justifyright', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_justifycenter', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_justifyfull', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_bold', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_italic', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_underline', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_strikethrough', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_bullist', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_numlist', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_sub', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_sup', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_anchor', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_link', 'mceButtonDisabled');
		tinyMCE.switchClass(toolbar_id + '_unlink', 'mceButtonDisabled');
		tinyMCE.switchClass(toolbar_id + '_outdent', 'mceButtonDisabled');
		tinyMCE.switchClass(toolbar_id + '_image', 'mceButtonNormal');
		tinyMCE.switchClass(toolbar_id + '_hr', 'mceButtonNormal');

		if (node.nodeName == "A" && tinyMCE.getAttrib(node, "class").indexOf('mceItemAnchor') != -1)
			tinyMCE.switchClass(toolbar_id + '_anchor', 'mceButtonSelected');

		// Get link
		var anchorLink = tinyMCE.getParentElement(node, "a", "href");

		if (anchorLink || any_selection) {
			tinyMCE.switchClass(toolbar_id + '_link', anchorLink ? 'mceButtonSelected' : 'mceButtonNormal');
			tinyMCE.switchClass(toolbar_id + '_unlink', anchorLink ? 'mceButtonSelected' : 'mceButtonNormal');
		}

		// Handle visual aid
		tinyMCE.switchClass(toolbar_id + '_visualaid', visual_aid ? 'mceButtonSelected' : 'mceButtonNormal');

		if (undo_levels != -1) {
			tinyMCE.switchClass(toolbar_id + '_undo', 'mceButtonDisabled');
			tinyMCE.switchClass(toolbar_id + '_redo', 'mceButtonDisabled');
		}

		// Within li, blockquote
		if (tinyMCE.getParentElement(node, "li,blockquote"))
			tinyMCE.switchClass(toolbar_id + '_outdent', 'mceButtonNormal');

		// Has redo levels
		if (undo_index != -1 && (undo_index < undo_levels-1 && undo_levels > 0))
			tinyMCE.switchClass(toolbar_id + '_redo', 'mceButtonNormal');

		// Has undo levels
		if (undo_index != -1 && (undo_index > 0 && undo_levels > 0))
			tinyMCE.switchClass(toolbar_id + '_undo', 'mceButtonNormal');

		// Select class in select box
		var selectElm = document.getElementById(toolbar_id + "_styleSelect");
		
		if (selectElm) {
			TinyMCE_AdvancedTheme._setupCSSClasses(toolbar_id);
		...
		do {
			...
			switch (alignNode.getAttribute('align').toLowerCase()) {
				case "left":
					tinyMCE.switchClass(toolbar_id + '_justifyleft', 'mceButtonSelected');
					breakOut = true;
				break;

				case "right":
					tinyMCE.switchClass(toolbar_id + '_justifyright', 'mceButtonSelected');
					breakOut = true;
				break;

				case "middle":
				case "center":
					tinyMCE.switchClass(toolbar_id + '_justifycenter', 'mceButtonSelected');
					breakOut = true;
				break;

				case "justify":
					tinyMCE.switchClass(toolbar_id + '_justifyfull', 'mceButtonSelected');
					breakOut = true;
				break;
			}
		...
		if (!setup_content) {
			...
			for (var i=0; i<ar.length; i+=2) {
				if (inst.queryCommandState(ar[i]))
					tinyMCE.switchClass(toolbar_id + ar[i+1], 'mceButtonSelected');
			}

			if (inst.queryCommandState("Underline") && (node.parentNode == null || node.parentNode.nodeName != "A"))
				tinyMCE.switchClass(toolbar_id + '_underline', 'mceButtonSelected');
		}
		...
		do {
			switch (node.nodeName) {
				case "UL":
					tinyMCE.switchClass(toolbar_id + '_bullist', 'mceButtonSelected');
				break;

				case "OL":
					tinyMCE.switchClass(toolbar_id + '_numlist', 'mceButtonSelected');
				break;

				case "HR":
					 tinyMCE.switchClass(toolbar_id + '_hr', 'mceButtonSelected');
				break;

				case "IMG":
				if (tinyMCE.getAttrib(node, 'name').indexOf('mce_') != 0 && tinyMCE.getAttrib(node, 'class').indexOf('mceItem') == -1) {
					tinyMCE.switchClass(toolbar_id + '_image', 'mceButtonSelected');
				}
				break;
			}
	...
	_setupCSSClasses : function(toolbar_id) {
		var i, selectElm;

		if (!TinyMCE_AdvancedTheme._autoImportCSSClasses)
			return;

		selectElm = document.getElementById(toolbar_id + '_styleSelect');
	...
	_insertImage : function(src, alt, border, hspace, vspace, width, height, align, title, onmouseover, onmouseout, className) {
